# Bloodhounds  
**Многопользовательская онлайн-игра с элементами стратегии**

---

## 🎮 Описание  
**Bloodhounds** — серверная часть игры, где игроки управляют собаками, собирают добычу и соревнуются за очки.  
**Функционал**:  
- Асинхронный HTTP-сервер на Boost.Beast.  
- Динамическая генерация игровых карт и добычи.  
- Сессионное управление игроками (токены, сохранение состояния).  
- Интеграция с PostgreSQL для хранения рекордов.  
- Сериализация данных в JSON (Boost.JSON).  

**Стек**:  
- **Язык**: C++20  
- **Библиотеки**: Boost (Asio, Beast, JSON), libpqxx  
- **База данных**: PostgreSQL  
- **Сборка**: Conan, CMake  
- **Деплой**: Docker  

**Цель проекта**: Создать масштабируемую серверную часть для онлайн-игры с возможностью автосохранения и интеграцией с БД.
...
Это каноничный способ запуска, которым будут пользоваться. Если он не работает, значит, мы что-то не так делаем.
